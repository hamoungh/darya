...
cvx_begin
       ...
       % minimize norm( sum(thetaV,2) - 0.8*cap ,1 ) +  r1 * pos( X_target - X_c);
cvx_end
thetaV
end % solve_nfm

function [thetaV,gammaV]=solve_nfm2()
        num_host=4;
        num_service=2; 
        cap=[8 8 8 8]';        
        RandStream.setDefaultStream ...
            (RandStream('mt19937ar','seed',sum(10)));
        cost=rand(num_host,1)*ones(1, num_service)+10;  
        util_per_service =  [9.0909 4.00]';  
       % c=rand(num_host, num_service)+10;  
...
cvx_begin
    ...
    thetaV>=0
    sum(thetaV,2)<=cap 
    sum(thetaV,1)' == util_per_service 
    minimize sum( sum(cost .* thetaV,2))
cvx_end
